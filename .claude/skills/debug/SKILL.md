...
```json
"scripts": {
    "//": "When modifying these scripts, remember to keep the skills in .claude up to date",
    ...
    "infra:dev:up": "docker compose --profile observability up --wait postgres redis clickhouse minio langfuse-web langfuse-worker -d",
    "infra:dev:stop": "docker compose --profile observability stop postgres redis clickhouse minio langfuse-web langfuse-worker",
    ...
    "format:diff": "git status --porcelain --untracked-files=all | awk '$1 !~ /D/ {print $NF}' | grep -E '\\.(ts)$' | grep -E '^(src|test)/' | xargs --no-run-if-empty prettier --write",
    "start": "nest start",
    ...
    "full": "npm run format:diff && tsc --noEmit && npm run lint && npm run test && npm run test:e2e",
    "prepare": "husky",
